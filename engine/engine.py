...
class SimulationEngine:
    ...
    def _apply_effects(self, outcome: Outcome, team: Team):
        ...
        if "discovered" in fx:
            # Scan revealed a node — add it and its neighbours to Red's known set
            disc = fx["discovered"]
            node_id = disc.get("node_id")
            if node_id:
                self.obs_builder.add_red_discovery(node_id)
                for neighbour in self.network.neighbors(node_id):
                    self.obs_builder.add_red_discovery(neighbour.id)
